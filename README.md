...
- **NPC names** — drawn above the bar, optionally at all times rather than only in combat, and
  optionally with the combat level. Long names can be truncated, and non-combat NPCs skipped.
...
| Damage Trail | Whether a lighter trail follows damage down the bar: Off, Match bar color, or Custom color. Healing has no trail. | Off |
...
| Combat NPCs Only | Excludes non-attackable NPCs from bars and names | On |
...
| Status Effects | How poison, venom, burns, disease and corruption show: Off, Bar tint, Icon, or Both. | Both |
